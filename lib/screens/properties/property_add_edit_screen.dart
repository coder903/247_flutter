...
class _PropertyAddEditScreenState extends State<PropertyAddEditScreen> {
  ...
  Future<void> _scanQrCode() async {
    try {
      final scannedCode = await BarcodeScannerService.scanBarcode(
        context: context,
        title: 'Scan QR Code',
        instructionText: 'Scan the system QR code',
      );
      if (scannedCode != null) {
        setState(() {
  ...
  Future<void> _saveProperty() async {
    ...
    try {
      ...
      print('Error saving system: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error saving system: $e')),
        );
      }
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return Scaffold(
        appBar: AppBar(
          title: Text(widget.property == null ? 'New System' : 'Edit System'),
        ),
          ...
          children: [
            ...
            ElevatedButton(
              ...
              child: _isSaving
                  ...
                  : Text(widget.property == null ? 'Create System' : 'Update System'),
            ),
            const SizedBox(height: 16),
          ],
        ),
